...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get the mentor ID from the form
    $mentorId = $_POST['id'];
    ...
    $stmt = $conn->prepare("UPDATE mentors SET email = ?, phone = ?, id_number = ?, occupation = ?, town_village = ?, province = ?, gender = ? WHERE id = ?");
    $stmt->bind_param("sssssssi", $email, $phone, $id_number, $occupation, $town_village, $province, $gender, $mentorId);

    // Execute the statement
    if ($stmt->execute()) {
        // Redirect back to mentor page or show success message
        header("Location: ../mentor.php?success=1");
    } else {
